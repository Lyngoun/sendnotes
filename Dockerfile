...
WORKDIR /app
COPY . .
# Copy vendor if it exists from base stage
RUN --mount=type=bind,from=base,source=/var/www/html,target=/mnt/base \
    if [ -d /mnt/base/vendor ]; then \
        cp -r /mnt/base/vendor /app/; \
    else \
        mkdir -p /app/vendor; \
    fi
